...
export function Dashboard() {
  ...
  const premium = info ? info.plan !== "free" : false;
  ...
  return (
    <div className="workspace-page">
      ...
      >
        ...
        <Link to="/study" className="btn-primary w-full sm:w-auto"><BookOpen size={16} aria-hidden />Study materials</Link>
      </motion.header>
      ...
      <div className="mt-5 grid gap-5 xl:grid-cols-[minmax(0,1fr)_18rem]">
        <section id="practice-library" className="surface-panel p-4 sm:p-6">
          ...
          <div className="subject-browser-workspace mt-5">
            <SubjectBrowser
              actions={(subject) => (
                <Link to={`/study?subject_id=${subject.id}`} className="btn-primary w-full px-3 py-2.5 text-xs" aria-label={`Study materials for ${subject.name}`}><BookOpenCheck size={14} aria-hidden />Study materials</Link>
              )}
            />
          </div>
        </section>
...
function PlanModule({ status, premium, info, onRetry }: { status: LoadStatus; premium: boolean; info: PlanInfo | null; onRetry: () => void }) {
  ...
  const title = info.plan === "pro" ? "Examify Pro" : info.plan === "plus" ? "Examify Plus" : info.canTakeExam ? "Free plan" : "Free exam used";
  const body = info.plan === "pro"
  ...
  return (
    <section className={`border-y p-5 ${premium ? "border-[#14274a] bg-[#14274a] text-white" : "border-[#14274a]/20 bg-[#fffdfa]/60 text-ink"}`}>
      ...
      <Link to="/upgrade" state={info.plan === "free" && !info.canTakeExam ? { reason: "free-limit" } : undefined} className={`mt-5 inline-flex items-center gap-1 text-sm font-extrabold ${premium ? "text-white" : "text-[#ce4040]"}`}>{premium ? "Manage plan" : "Explore plans"}<ArrowRight size={15} aria-hidden /></Link>
    </section>
  );
}
